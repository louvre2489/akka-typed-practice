...
import practice.akka.iot.DeviceGroupQuery.WrappedRespondTemperature
import practice.akka.iot.DeviceManager.{DeviceNotAvailable, DeviceTimedOut, RespondAllTemperatures, Temperature, TemperatureNotAvailable}
...
class DeviceGroupQuerySpec extends ScalaTestWithActorTestKit with WordSpecLike {
  ...
  "DeviceGroupQuery" must {
    "return temperature value for working devices" in {
      ...
      val device1 = createTestProbe[Device.DeviceCommand]()
      val device2 = createTestProbe[Device.DeviceCommand]()

      val deviceIdToActor = Map("device1" -> device1.ref, "device2" -> device2.ref)

      val queryActor =
        spawn(DeviceGroupQuery(deviceIdToActor, requestId = 1, requester = requester.ref, timeout = 3.second))
